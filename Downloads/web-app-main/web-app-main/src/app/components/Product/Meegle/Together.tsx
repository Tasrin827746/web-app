...
const Together = () => {
    const [activeRole, setActiveRole] = useState<string>('Project Manager');
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <HeadingComponent headingLabel="Together, Every Step of the Way" />
        <HeadingDescriptionComponent
          headingDescriptionLabel="Meeple suits the diverse needs of team members, amplifying collective potentials."
        />
      </header>
      ...
      <div className="flex justify-center space-x-4">
        {roles.map(role => (
          ...
          >
            <Image width={32} height={32} src={role?.icon} alt={`${role?.name} Icon`} className="w-8 h-8 mb-2" />
            {role?.name}
          </button>
        ))}
      </div>
      ...
      <button
        className="mt-8 px-6 py-3 bg-gradient-to-l from-purple-300 to-purple-500 hover:bg-gradient-hover text-white text-xl font-semibold rounded-full shadow transition flex items-center"
      >
